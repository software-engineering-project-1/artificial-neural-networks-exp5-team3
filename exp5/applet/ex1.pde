...
void drawENERGYdiagram(staTES[] stateARRR, int[] orderSTATESS)
{
  ...
  for (int i=0;i<orderSTATESS.length;i++)
  {
    ...
    for (int j=0;j<3;j++)
    {
      tt[iC] = int(cTEMP[j])-48;
    }
    int j = stateARRR[i].orDER; 
    count =0;
    for (int y=0;y<i;y++)
    {
      if (stateARRR[y].orDER == j)// && stateARRR[y].printed==false)
      {
        count++;
      }
    }     
    ...
    fill(50);
    rect(drawStartXX, drawStartY-(65*j), 90, 45);
    fill(255);
    textSize(15);

    text(tt[0], drawStartXX+40, drawStartY-(65*j)+18);
    text(tt[1], drawStartXX+5, (drawStartY-(65*j))+34);
    text(tt[2], drawStartXX+75, (drawStartY-(65*j))+34);

    text("{"+stateARRR[i].enerGY+"}", drawStartXX+24, (drawStartY-(65*j))+35);
    stateARRR[i].stateCOX = drawStartXX;
    stateARRR[i].stateCOY = drawStartY-(65*j);
